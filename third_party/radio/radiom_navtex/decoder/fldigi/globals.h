...
// (const dropped — gcc puts const arrays in .rodata, then our static-
// init filler segfaults trying to const_cast-write. fldigi never mutates
// this table at runtime, so the API contract is preserved.)
extern struct mode_info_t mode_info[NUM_MODES];

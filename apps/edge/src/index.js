...
async function fetchUpstream(url) {
  const response = await fetch(url, {
    headers: { 'User-Agent': 'SeaCommons-Edge/0.1 (+https://seacommons.org)' },
...
async function environmentResponse(request, env, context, url) {
  ...
  // One-hundredth degree is below the nominal marine product resolution while
  // still allowing nearby requests to share an edge snapshot.
  ...
  try {
    ...
    const [weather, marine] = await Promise.all([
      fetchUpstream(urls.weather),
      fetchUpstream(urls.marine),
    ]);
...
export default {
  async fetch(request, env, context) {
    ...
    if (url.pathname === '/health') {
      return json({ status: 'ok', service: 'seacommons-edge', simulation: 'client-worker' }, 200, corsHeaders(request, env));
    }

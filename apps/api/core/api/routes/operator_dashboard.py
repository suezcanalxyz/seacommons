...
function renderFleet(data){
 const b=$("sarFleet"),rank={live:0,stale:1,offline:2};
 b.textContent="";
 const rows=(data.features||[])
   .map(f=>{const p=f.properties||{},ts=p.position_timestamp_utc||p.last_seen_utc||p.last_seen||p.timestamp||p.updated_at,state=p.ais_status||"offline";return{p,ts,state}})
   .sort((a,b)=>(rank[a.state]??9)-(rank[b.state]??9)||String(a.p.ship_name||"").localeCompare(String(b.p.ship_name||"")));
 rows.forEach(x=>{
   const d=document.createElement("div");d.className="fleetAsset";
   const n=document.createElement("b");n.textContent=x.p.ship_name||x.p.vessel_name||x.p.mmsi||"SAR asset";
   const o=document.createElement("small");o.textContent=(x.p.org||x.p.organisation||x.p.operator_type||"")+" · "+(x.p.operator_type==="state_authority"?"state SAR":"civil SAR");
   const st=document.createElement("small");st.className="fleetState "+x.state;st.textContent=x.state.toUpperCase()+" · AIS "+(x.ts?ago(x.ts):"not available")+(x.p.position_policy==="stale_withheld"?" · map position withheld":"");
   d.append(n,o,st);b.append(d)
 });
 if(!rows.length)b.innerHTML="<div class='fleetAsset'>No SAR registry rows available.</div>";
 const live=rows.filter(x=>x.state==="live").length,stale=rows.filter(x=>x.state==="stale").length,tr=data.meta?.tracking_stream||{};
 $("fleetFresh").textContent="current fixes only · "+live+" live · "+stale+" stale · "+(rows.length-live-stale)+" offline · tracker "+(tr.connected?"connected":"degraded")
}

...
@media(max-width:1300px){.funnel{grid-template-columns:repeat(4,1fr)}.sensors{grid-template-columns:repeat(3,1fr)}.logs{grid-template-columns:1fr}.casegrid{grid-template-columns:1fr}.bottom{grid-template-columns:1fr}}@media(max-width:650px){main{padding:12px}.funnel{grid-template-columns:1fr 1fr}.sensors{grid-template-columns:1fr 1fr}.health{max-width:45%}}
</style>
</head>
<body>
...
function sourceRows(obj){let b=$("sources");b.textContent="";Object.entries(obj||{}).slice(0,24).forEach(([n,c])=>{let d=document.createElement("div");d.className="source";let x=document.createElement("b");x.textContent=n;let y=document.createElement("small");y.textContent=fmt(c);d.append(x,y);b.append(d)})}

...
function renderFleet(data){let b=$("sarFleet");b.textContent="";let rows=(data.features||[]).map(f=>{let p=f.properties||{},ts=p.position_timestamp_utc||p.last_seen_utc||p.last_seen||p.timestamp||p.updated_at,age=ts?Math.max(0,(Date.now()-new Date(ts).getTime())/1000):Infinity,state=age<=600?"live":age<=3600?"stale":"offline";return{p,ts,age,state}}).sort((a,b)=>({live:0,stale:1,offline:2}[a.state]-({live:0,stale:1,offline:2}[b.state])||String(a.p.ship_name||"").localeCompare(String(b.p.ship_name||"")));rows.forEach(x=>{let d=document.createElement("div");d.className="fleetAsset";let n=document.createElement("b");n.textContent=x.p.ship_name||x.p.vessel_name||x.p.mmsi||"SAR asset";let o=document.createElement("small");o.textContent=(x.p.org||x.p.organisation||x.p.operator_type||"")+" · "+(x.p.operator_type==="state_authority"?"state SAR":"civil SAR");let st=document.createElement("small");st.className="fleetState "+x.state;st.textContent=x.state.toUpperCase()+" · AIS "+(x.ts?ago(x.ts):"not available");d.append(n,o,st);b.append(d)});if(!rows.length)b.innerHTML="<div class='fleetAsset'>No SAR registry rows available.</div>";let live=rows.filter(x=>x.state==="live").length,stale=rows.filter(x=>x.state==="stale").length;$("fleetFresh").textContent="AIS freshness only · "+live+" live · "+stale+" stale · "+(rows.length-live-stale)+" offline"}
async function refreshFleet(){try{renderFleet(await get("/api/v1/live/ngo-vessels"))}catch(err){$("fleetFresh").textContent="fleet inventory degraded · "+String(err.message||err)}}
...
refresh();refreshOverall();refreshFleet();setInterval(refresh,5000);setInterval(refreshFleet,30000);setInterval(refreshOverall,60000);
</script>
</body></html>"""

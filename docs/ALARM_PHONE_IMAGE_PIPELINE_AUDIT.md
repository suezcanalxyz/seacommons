...
| 13 — promote V2 out of shadow | operator-gated | The structured V2 pipeline is already the live path for the Alarm Phone handles (`ALARM_PHONE_IMAGE_V2_ENABLED=true` by default). What remains is an **operator decision**, not code: (1) build a local `benchmark/alarm_phone/` set from real public tweets, (2) run `--benchmark` and confirm V2's false-coordinate rate ≤ V1's, (3) add relay handles to `ALARM_PHONE_IMAGE_V2_ACCOUNTS` (optionally via `ALARM_PHONE_IMAGE_V2_SHADOW=true` first), (4) review the shadow `image_assessment_shadow` deltas, then clear the shadow flag. No further code change is required for the cutover. |

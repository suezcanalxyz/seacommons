...
- [x] **Step 1: Write RED tests for profile-independent normalization and forbidden targets**
...
- [x] **Step 3: Implement the pure policy types and classifiers**
...
- [x] **Step 1: Write RED tests for pinning and Host/SNI preservation**
...
- [x] **Step 2: Write RED redirect and response-bound tests**
...
- [x] **Step 4: Implement pinned request execution and manual redirect loop**
...
- [x] **Step 1: Write RED SSRF regressions for `/extract-image` and media helpers**
...
- [x] **Step 3: Migrate the two acquisition paths**
...
- [x] **Step 1: Write RED internal-origin tests**
...
- [x] **Step 3: Implement the migrations**
...
- [x] **Step 1: Write RED auth/JWKS tests**
...
- [x] **Step 3: Implement canonical JWKS fetch and key selection**
...
- [x] **Step 3: Implement scanner and manually snapshot remaining legacy calls**
...
```
- [x] **Step 4: Update controller/testing docs**
...
- [x] **Step 5: Run focused security release gates**
...
- [x] **Step 6: Run full exact-head gates**
...
- [x] **Step 7: Validate production configuration without revealing configured origins**
...
- [x] **Step 8: Production smoke after merge/restart**
...
- [x] **Step 9: Final exact-diff review and commit controller evidence**
...
- Accepted/closed on release-qualified `main` `6202aa2`.
- PR #164 merged the canonical outbound HTTP/SSRF boundary; PR #165 merged the semantics-preserving Play catalog performance prefilter.
- Final manual Full CI run `34755443674`: `repository`, `api`, `web`, `edge`, `browser-e2e`, and `browser-production-smoke` all PASS.
- Final API gate: `1715 passed, 2 skipped`; Humanitarian contract gate: `58 passed`.
- Production `/health` = `ok`; `/ready` = database `ok`.
- Production outbound smoke: public HTTPS asset returns 200 through `PUBLIC_UNTRUSTED`; loopback HTTPS target is rejected as `blocked_target`.
- Play production after the prefilter: `226` public records (`54` Humanitarian, `172` Maritime), first page returns all 226 unique records, `total_count=226`, `next_offset=null`; measured first-page latency about `4.65 s` versus about `20.5 s` before the prefilter.
- Alarm Phone lifecycle/archival verification: resolved Pozzallo and Crete incidents are absent from operational Humanitarian Live and present in Play. Crete media-pin reprocessing stores `34.10776,25.10054` with about `70.5 km` uncertainty and `machine_ocr_unverified`.
- Remaining legacy direct HTTP calls are frozen at the checked-in inventory (`29` path/callee entries, `44` calls); new bypasses fail CI. WebSocket transports and later fixed-collector migrations remain follow-up scope.

...
| 10 — coverage-aware gap (`gap` vs `coverage_gap`) | *this branch* — landed. `AISAnomalyDetector._silence_sweep_loop` no longer emits a bare `gap` on silence alone (GP-1). `_build_gap_event` first runs `_coverage_around(lat, lon)` — how many vessels within `_GAP_NEIGHBOUR_RADIUS_NM` (40 nm) were reporting within the last 6 h (`before`) versus still reporting within the last 15 min (`after`). When `before ≥ 3` and `after == 0` the surrounding coverage collapsed at the same time, so the event is a `coverage_gap`: `severity=low`, `maritime_domain=safety`, `report_kind=coverage_outage`, `linked_mmsi=None`, a "reception/source outage, not intentional dark activity" title/text, and — via `_emit`'s cooldown key and `_to_intel_event`'s id both collapsing to a `~1°` grid cell — **one** event for the whole area, never N vessel-specific gaps (audit invariant). Otherwise it stays a vessel `gap`, with confidence scaled down when no fresh neighbour confirms healthy local coverage. Every gap carries `nearby_vessels_before` / `nearby_vessels_after` / `local_reporting_ratio` evidence and the `assess_ais_gap` EventAssessment. `tests/test_ais_anomaly.py` +4 (healthy→`gap`, simultaneous silence→`coverage_gap`, neutral coverage-outage intel event, feed-wide outage collapses to one). Backend suite green (692 passed). Frontend already labels `coverage_gap` "AIS coverage outage" (PR 7). |
